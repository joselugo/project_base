...
class Class_seguridad
{
    ...
    /**
     * Asigna una clave de acceso numérica a un módulo específico.
     * Obtiene la clave de acceso asociada a un módulo.
     * el módulo hace referencia al permiso creado en tabla permisos de la base de datos
     * @param string $modulo Nombre del módulo (permiso). 
     * @return int Clave de acceso (id del permiso).
     */
    function key_access($modulo = '')
    {
        $clave = 0; // Por defecto, se establece la clave como 0.
        switch ($modulo) {
            case "HOME":
                $clave = 1;
                break;
    ...
    /**
     * Verifica el nivel de acceso de un usuario a un permiso específico.
     *
     * @param int $id_usuario ID del usuario que se está verificando.
     * @param int $id_permiso ID del permiso que se está verificando.
     * @return int Nivel de acceso:
     *             - 0: Denegado (el usuario no tiene acceso al permiso).
     *             - 1: Lectura (el usuario tiene permiso de lectura).
     *             - 2: Escritura (el usuario tiene permiso de escritura).
     *             - 3: Eliminar (el usuario tiene permiso de eliminar).
     */
    function check_permission($id_usuario, $id_permiso)
    {
        $tipo_acceso = 0; // Inicialmente, el acceso se establece como denegado (0).

        // Obtiene los permisos asociados al usuario y al permiso en cuestión.
        $permisos = $this->mdl_security->verificar_permiso($id_usuario, $id_permiso);

        if ($permisos != false) {
            // Itera a través de los permisos y determina el nivel de acceso máximo.
            foreach ($permisos as $permiso) {
                $tipo_acceso_temp = 0;

                // Comprueba si el permiso es de usuario o rol y obtiene el nivel de acceso correspondiente.
                if ($permiso->permiso_usuario < 0) {
                    $tipo_acceso_temp = (int)$permiso->permiso_rol;
                } else {
                    $tipo_acceso_temp = (int)$permiso->permiso_usuario;
                }
                // Actualiza el nivel de acceso si el nivel actual es mayor.
                if ($tipo_acceso_temp > $tipo_acceso) {
                    $tipo_acceso = $tipo_acceso_temp;
                }
            }
    ...
    /**
     * Controlador para redirigir a la página de inicio de sesión ("login").
     *
     * Esta función se encarga de cargar la vista de inicio de sesión y mostrarla al usuario.
     * También configura la imagen de fondo de la página de inicio de sesión recuperando datos de la base de datos.
     * 
     * @return bool Siempre devuelve `false` ya que esta función se utiliza únicamente para mostrar la vista.
     */
    public function login_back()
    {
        // Se define un arreglo con un único elemento, el valor 64.
        $ids = array(64);

        // Se llama al modelo mdl_conf para obtener la configuración de la imagen de fondo.
        $img_db = $this->mdl_conf->get_configuracion($ids);
